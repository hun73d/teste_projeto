 ...
 * 
 * ATENÇÃO
 * A PALAVRA "Review" DEVERÁ SER ALTERADA POR "Anuncio" (assim como a classe 
 * Review.java e seu conteúdo) nesse branch experimental
 * 
 * -Não esquecer de colocar o Goal do Maven como jetty:run (a menos que rode no
 * Tomcat 9.0)
 * 
 * 
 * 
    ...
    private static class SingletonHolder {
        ...
        private static ReviewService createDemoReviewService() {
            ...
            for (int i = 0; i < reviewCount; i++) {
                ...
                Categoria category = CategoryService.getInstance()
                ...
                reviewService.saveAnuncio(review);
            }
...
/*
    ...
    !
    !
    !private Map<Long, Review> review = new HashMap<>(); -->linha original
    !private Map<Long, Review> anuncios = new HashMap<>(); -->linha nova 
    !       Observe que o botão de criar review foi alterado para criar anúncio
    !
    */
    private Map<Long, Review> anuncios = new HashMap<>();
    ...
    public List<Review> findReviews(String filter) {
        ...
        return anuncios.values().stream().filter(
     ...
     */
    public boolean deleteAnuncio(Review review) {
        return anuncios.remove(review.getId()) != null;
    }
     ...
     *            the review to save
     */
    public void saveAnuncio(Review dto) {
        Review entity = anuncios.get(dto.getId());
        Categoria category = dto.getCategory();
        ...
        if (entity == null) {
            ...
            anuncios.put(entity.getId(), entity);
        } else {
            entity.setScore(dto.getScore());
            entity.setName(dto.getName());
            entity.setAnunciante(dto.getAnunciante());
