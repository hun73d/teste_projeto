...
public class CategoriesList extends VerticalLayout {
    ...
    private final Grid<Categoria> grid = new Grid<>();
    ...
    private void addSearchBar() {
        ...
        newButton.addClickListener(e -> form.open(new Categoria(),
    ...
    private void addContent() {
        ...
        grid.addColumn(Categoria::getName).setHeader("Nome").setWidth("8em").setResizable(true);
    ...
    private Button createEditButton(Categoria category) {
    ...
    private String getReviewCount(Categoria category) {
    ...
    private void updateView() {
        List<Categoria> categories = CategoryService.getInstance()
                .findCategories(searchField.getValue());
        grid.setItems(categories);

        if (searchField.getValue().length() > 0) {
            header.setText("Buscar por “"+ searchField.getValue() +"”");
        } else {
            header.setText("Categorias");
        }
    }

    private void saveCategory(Categoria category,
            AbstractEditorDialog.Operation operation) {
        CategoryService.getInstance().saveCategory(category);

        Notification.show(
                "Categoria " + operation.getNameInText() + "salva.", 3000, Position.BOTTOM_START);
        updateView();
    }

    private void deleteCategory(Categoria category) {
        List<Review> reviewsInCategory = ReviewService.getInstance()
                .findReviews(category.getName());
        ...
        reviewsInCategory.forEach(review -> {
            ...
            ReviewService.getInstance().saveAnuncio(review);;
        });
        CategoryService.getInstance().deleteCategory(category);

        Notification.show("Categoria apagada com sucesso.", 3000, Position.BOTTOM_START);
        updateView();
    }
}
